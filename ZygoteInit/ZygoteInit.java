...
public class ZygoteInit {
     ...
     */
    // socket  zygote stream  666 (init.rc定义)
    private static void registerZygoteSocket() {
        if (sServerSocket == null) {
            int fileDesc;
            try {
                String env = System.getenv(ANDROID_SOCKET_ENV);  // 定义为 "ANDROID_SOCKET_zygote"
            ...
            try {
                // 创建套接字文件描述，并与 /dev/socket/zygote 绑定
    ...
    static void preload() {
        preloadClasses(); // 加载应用程序 Framework 中的类
        preloadResources(); // 加载应用程序 Framework 中的资源
        preloadOpenGL();
    }
     ...
     */
    // 加载应用程序 Framework 中的类
    private static void preloadClasses() {
        final VMRuntime runtime = VMRuntime.getRuntime();

        // 获取文件 preloaded-classes 的输入流
        InputStream is = ClassLoader.getSystemClassLoader().getResourceAsStream(
                PRELOADED_CLASSES); // "preloaded-classes"
        if (is == null) {
            ...
            try {
                ...
                while ((line = br.readLine()) != null) {
                    ...
                    try {
                        ...
                        Class.forName(line);  // 将对应的类加载到内存中(加载类信息)
     ...
     */
    // 加载应用程序 Framework 中的资源(字符串、颜色、图像文件、音频文件)
    private static void preloadResources() {
        ...
        try {
            System.gc();
            runtime.runFinalizationSync();
            mResources = Resources.getSystem();  // 系统资源对象
            mResources.startPreloading(); // 加载资源
    ...
    private static boolean startSystemServer()
        ...
        String args[] = {
            ...
            "com.android.server.SystemServer",  // SystemServer 类
        };
        ...
        try {
            ...
            // 与其他应用程序不同，SystemServer 调用 forkSystemServer 方法来创建新进程(SystemServer)
            // SystemServer 进程必须能正常工作
        ...
        /* For child process */
        if (pid == 0) {
            // SystemServer进程中运行 com.android.server.SystemServer 类中的 main()方法。main中会加载 android_servers 本地库(apk中jni)
    ...
    public static void main(String argv[]) {
        try {
            ...
            // 加载Android Application Framework 使用的类与资源、OpenGL
            preload();
            ...
            // 运行 SystemServer ,Android平台需要的本地服务

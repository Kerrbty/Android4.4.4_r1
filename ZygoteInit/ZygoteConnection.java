...
class ZygoteConnection {
    ...
    boolean runOnce() throws ZygoteInit.MethodAndArgsCaller {
        ...
        try {
            ...
            // 设置应用程序的gid、uid、调试标志、rlimit、以及检查运行权限等
            ...
            // 创建新进程(native函数fork，然后设置uid（--setuid），gid（--setgid），rlimit（--rlimit=r，c，m）等)
        ...
        try {
            if (pid == 0) {
                // in child
                // 加载新进程所需要的类，并调用类的main方法，启动新的应用程序。
                ...
                // Zygote进程返回新建进程是否成功，成功返回pid。请求完成断开连接，关闭socket
